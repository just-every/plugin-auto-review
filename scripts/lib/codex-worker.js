...
async function runReview({
  ...
  return runReviewer({
    snapshotDir,
    workerDir: path.join(jobDir, "reviewer"),
    prompt: buildReviewPrompt({ snapshotDir, changedPaths, diff }),
    changedPaths,
    model,
    reasoning,
    serviceTier
...
function runReviewer({ snapshotDir, workerDir, prompt, changedPaths, model, reasoning, serviceTier }) {
  return new Promise((resolve) => {
    ensureDir(workerDir);
    const schemaPath = path.join(workerDir, "review-schema.json");
    const lastMessagePath = path.join(workerDir, "last-message.json");
    ...
    const child = childProcess.spawn(codexBin(), args, {
      ...
      env: {
        ...process.env,
        AUTO_REVIEW_CHILD: "1",
        AUTO_REVIEW_CHILD_CWD: workerDir,
    ...
    const timeout = setTimeout(() => {
      finish(failedReview(`review timed out after ${timeoutMs}ms`, stdout, stderr));
      child.stdin.destroy();
    ...
    child.on("error", (error) => {
      finish(failedReview(`failed to spawn codex: ${error.message}`, stdout, stderr));
    });
    child.on("close", (code, signal) => {
      if (code !== 0) {
        finish(failedReview(`codex exited with ${code ?? signal}`, stdout, stderr));
        return;
      }
      let parsed;
      try {
        ...
        finish(failedReview(`could not read review JSON: ${error.message}`, stdout, stderr));
        return;
      }
      const validation = validateReviewResult(parsed, { changedPaths });
      if (!validation.ok) {
        finish(failedReview(`review JSON failed schema: ${validation.errors.join("; ")}`, stdout, stderr));
        return;
      }
      finish({
        status: parsed.findings.length > 0 ? "findings" : "clean",
...
function reviewTimeoutMs() {
  const raw = process.env.AUTO_REVIEW_TIMEOUT_MS;
  if (!raw) return REVIEW_TIMEOUT_MS;
  const parsed = Number(raw);
  if (!Number.isFinite(parsed) || parsed < 1) {
    throw new Error("AUTO_REVIEW_TIMEOUT_MS must be a number >= 1");
  }
...
function failedReview(error, stdout, stderr) {
  return {
    status: "failed",
    failures: [
      {
