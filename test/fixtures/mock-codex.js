...
if (process.env.MOCK_CODEX_ENV_LOG) {
  fs.appendFileSync(
    process.env.MOCK_CODEX_ENV_LOG,
    `${JSON.stringify({
      AUTO_REVIEW_CHILD: process.env.AUTO_REVIEW_CHILD || null,
      AUTO_REVIEW_CHILD_CWD: process.env.AUTO_REVIEW_CHILD_CWD || null,
      AUTO_REVIEW_SNAPSHOT_DIR: process.env.AUTO_REVIEW_SNAPSHOT_DIR || null
    })}\n`,
    "utf8"
  );
}
...
process.stdin.on("end", () => {
  ...
  if (process.env.MOCK_CODEX_HANG === "1") {
    setInterval(() => {}, 1000);
    return;
  }
